(function () {
    ...
    function modifyStyles(doc) {
        ...
        forEach(doc.querySelectorAll("style"), function (style) {
            if (!tweaker.tweakStyleText(style.innerText))
                return;
    ...
    proto.tweakStyleText = function (text) {
        this.text = text;
        this.tweakPlatform();
    ...
    if ('webkitWritingMode' in style) {
        proto.prefix = "-webkit-";
        proto.tweakPlatform = function tweakWebKit() {
        ...
        proto.tweakPlatform = function tweakIE() {
